...
// $data=Array(
//     $onet[0]=>['color'=>[24,203,238],'value'=>$f_score[0]],
//     $onet[1]=>['color'=>[172,13,26],'value'=>$f_score[1]],
//     $onet[2]=>['color'=>[247,253,4],'value'=>$f_score[2]],
//     $onet[3]=>['color'=>[10,25,100],'value'=>$f_score[3]],
//     $onet[4]=>['color'=>[0,25,10],'value'=>$f_score[4]],
//     $onet[5]=>['color'=>[24,0,238],'value'=>$f_score[5]]
// );
...
// Interest Mapper
    ...
    while($row_apt = mysqli_fetch_array($res_apt))
    {
        ...
        //echo "Profession :".$cd." - ";
        /** Commented by Sudhir
        $ar_w = str_replace("%","",$row_apt['AR_w'])/100;
        echo "AR Modified :".$ar_w."<br>";
        $vr_w = str_replace("%","",$row_apt['VR_w'])/100;
        echo "VR Modified :".$vr_w."<br>";
        $sa_w = str_replace("%","",$row_apt['SA_w'])/100;
        $com_w = str_replace("%","",$row_apt['COM_w'])/100;
        $nc_w = str_replace("%","",$row_apt['NC_w'])/100;
        $om_w = str_replace("%","",$row_apt['OM_w'])/100;
        ...
        **/
        ...
        $ar_w,$vr_w,$sa_w,$com_w,$nc_w,$om_w);
        $sql_update = "update career_sui_latest set apt='$match' where profession_name='$cd' and code='$code'";
        mysqli_query($con,$sql_update);
    }
    function aptitude_mapper($a1,$a2,$a3,$a4,$a5,$a6,$b1,$b2,$b3,$b4,$b5,$b6,$c1,$c2,$c3,$c4,$c5,$c6)
    {
    ...
    // function recommendation_mapper(array $arr, $a,$b)
    // {
    //     $match = '';
    //     /*
    //     $arr_level = array('VHVH','H1VH','H2VH','H3VH','MHVH','MVH','LVH','VLVH','VHH1',
    //     'H1H1','H2H1','H3H1','MHH1','MH1','LH1','VLH1','VHH2','H1H2','H2H2','H3H2','MHH2',
    //     'MH2','LH2','VLH2','VHM','H1M','H2M','H3M','MHM','MM','LM','VLM','VHL','H1L','H2L',
    //     'H3L','MHL','ML','LL','VLL','LVL');

    //     $arr_recomm = array('Top Choice','Good Choice','Good Choice','Optional','Optional'
    //     ,'Optional','Avoid','Avoid','Top Choice','Good Choice','Good Choice','Optional'
    //     ,'Optional','Optional','Avoid','Avoid','Good Choice','Good Choice','Good Choice','Optional','Optional'
    //     ,'Optional','Avoid','Avoid','Develop','Develop','Develop','Develop','Develop','Develop'
    //     ,'Avoid','Avoid','Explore','Explore','Explore','Explore','Explore','Explore','Avoid','Avoid','Avoid');
    //     **/
       
    //     $x = "I-".$a."-A-".$b;
    //     foreach($arr as $ar_name)
    //     {
    //         if($ar_name[5] == $x)
    //         {
    //             $match = $ar_name[4];
...
/* 20-01-2022
SELECT a.recommendation, COUNT(a.profession_name),b.11th_12th  FROM career_sui_latest a LEFT JOIN career_edu_latest b ON b.profession_name = a.profession_name
WHERE a.code='ACEE870066' AND (a.recommendation !='Avoid' AND a.Cluster !='#N/A') AND b.display_priority = 1
AND b.11th_12th IS NOT NULL AND b.11th_12th != '#NAME?'
GROUP BY  a.recommendation,b.11th_12th 
ORDER BY COUNT(a.profession_name) DESC
----------------->>>>
SELECT a.Cluster, a.recommendation, COUNT(a.profession_name)  FROM career_sui_latest a LEFT JOIN career_edu_latest b ON b.profession_name = a.profession_name
WHERE a.code='ACEE870066' AND (a.recommendation !='Avoid' AND a.Cluster !='#N/A') AND b.display_priority = 1
AND b.11th_12th IS NOT NULL AND b.11th_12th != '#NAME?'
GROUP BY a.Cluster, a.recommendation
ORDER BY COUNT(a.profession_name) DESC
---------------->>>>>>>>
SELECT a.Cluster, a.recommendation, COUNT(a.profession_name),b.11th_12th  FROM career_sui_latest a LEFT JOIN career_edu_latest b ON b.profession_name = a.profession_name
WHERE a.code='ACEE870066' AND (a.recommendation !='Avoid' AND a.Cluster !='#N/A') AND b.display_priority = 1 AND b.11th_12th IS NOT NULL AND b.11th_12th != '#NAME?'
GROUP BY a.Cluster, a.recommendation,b.11th_12th
ORDER BY COUNT(a.profession_name) DESC,b.11th_12th,a.Cluster
----------------->>>>>>>>>>>
SELECT a.Cluster, a.recommendation, COUNT(a.profession_name)  FROM career_sui_latest a LEFT JOIN career_edu_latest b ON b.profession_name = a.profession_name
WHERE a.code='ACEE829236' AND (a.recommendation ='Top Choice' AND a.Cluster !='#N/A') AND b.display_priority = 1
AND b.11th_12th IS NOT NULL AND b.11th_12th != '#NAME?'
GROUP BY a.Cluster, a.recommendation
ORDER BY  a.recommendation,COUNT(a.profession_name) DESC
-------------->>>>>>>>>>>>>>
SELECT a.Cluster, a.recommendation, a.profession_name FROM career_sui_latest a LEFT JOIN career_edu_latest b ON b.profession_name = a.profession_name
WHERE a.code='ACEE829236' AND (a.recommendation ='Top Choice' AND a.Cluster !='#N/A') AND b.display_priority = 1
AND b.11th_12th IS NOT NULL AND b.11th_12th != '#NAME?'
ORDER BY  a.Cluster
...
code start here by test............ <br>";
   ...
   // var_dump($arr_careers);
    ...
    // Top Cluster identification - Top Choice, Good Choice
    {
     /*   
    
        $sql_cnt = "SELECT Cluster, COUNT(profession_name) FROM career_sui_latest";
        $sql_cnt .= " WHERE recommendation IN ('Top Choice', 'Good Choice') and code='$code'";
        $sql_cnt .= " GROUP BY Cluster";
        $sql_cnt .= " ORDER BY COUNT(profession_name) DESC";
        $res_cnt = mysqli_query($con,$sql_cnt);
        
        $num_rows = mysqli_num_rows($res_cnt);
        if($num_row >0)
        {
            while($row = mysqli_fetch_array($res_cnt))
            {
                array_push($arr_a, $row);
                
            }
        }
        
        echo "Top Clusters (Top Choice, Good Choice) :<pre>";
        print_r($arr_a);
        
        echo "Total No of Career Options (Top Choice, Good Choice) :".$num_rows."<br>";
        if($num_rows <$careers_to_disp)
    {
        $opt_rows = $careers_to_disp - $num_rows;
        
        $sql_cnt_1 = "SELECT Cluster, COUNT(profession_name) FROM career_sui_latest";
        $sql_cnt_1 .= " WHERE recommendation IN ('Optional') and code='$code'";
        $sql_cnt_1 .= " GROUP BY Cluster";
        $sql_cnt_1 .= " ORDER BY COUNT(profession_name) DESC";
        $sql_cnt_1 .=" LIMIT $opt_rows";
        $res_cnt_1 = mysqli_query($con,$sql_cnt_1);
        
        $num_rows_1 = mysqli_num_rows($res_cnt_1);
        echo "Total No of Career Options (Optional) :".$num_rows_1."<br>";
        
        
        if($num_rows + $num_rows_1 <$careers_to_disp)
        {
            if($num_rows_1 >0)
            {
               while($row = mysqli_fetch_array($res_cnt_1))
                {
                    array_push($arr_a, $row);
            ...
            echo "Top Clusters (Optional) :<pre>";
            print_r($arr_a);
            
            $dev_exp_rows = $careers_to_disp - $num_rows - $num_rows_1;
            
            $sql_cnt_2 = "SELECT Cluster, COUNT(profession_name) FROM career_sui_latest";
            $sql_cnt_2 .= " WHERE recommendation IN ('Explore','Develop') and code='$code'";
            $sql_cnt_2 .= " GROUP BY Cluster";
            $sql_cnt_2 .= " ORDER BY COUNT(profession_name) DESC";
            $sql_cnt_2 .=" LIMIT $dev_exp_rows";
            $res_cnt_2 = mysqli_query($con,$sql_cnt_2);
            
            $num_rows_2 = mysqli_num_rows($res_cnt_2);
            echo "Code :".$code." - Total No of Career Options ('Explore','Develop') :".$num_rows_2."<br>";
            if($num_rows_2 > 0)
            {
                while($row = mysqli_fetch_array($res_cnt_2))
                {
                    array_push($arr_a, $row);
            ...
            echo "Top Clusters (Explore, Develop) :<pre>";
            print_r($arr_a);
    ...
    */
    ...
    
    {
       /*
        ...
        foreach($choise as $ch)
        {
            echo "Client Choice :".$ch."<br>";
            //$sql_choice = "select * from career_sui_latest where recommendation='$ch' and code='$code'";
            
            $sql_choice = "select * from career_sui_latest where Cluster IN ({$top_clusters}) AND recommendation='$ch' and code='$code'";
            ...
            while($row_choice = mysqli_fetch_array($res_choice))
            {
                    ...
                    //Added by Sudhir on 24 Oct 2021
                    $prof_name = $row_choice['profession_name'];
                    $sql_com = "select common_profession from career_sui_temp_latest WHERE profession_name = '$prof_name'";
                    $com_choice = mysqli_query($con,$sql_com);
                    $x = mysqli_fetch_array($com_choice)[0];
                    //echo "Profession_Type :".$x."<br>";
                    
                    //End of Added by Sudhir on 24-Oct-2021
                    ...
                    while($row_a_d = mysqli_fetch_array($res_all_data))
                    {
                        ...
                        if($x == 1 &&  $i<=$careers_to_disp)
                        {
                            //array_push($t,array($row_a_d['profession_name'],$row_a_d['Cluster'],$row_a_d['11th_12th'],$row_a_d['Education'],$ch));
                        }
    ...
    /*
    foreach($arr_careers as $rec_car)
    {
        $p_ct +=1;
        array_push($t,array($rec_car['profession_name'],$rec_car['Cluster'],$rec_car['11th_12th'],$rec_car['Education'],$rec_car['recommednation']));
        if($p_ct >$careers_to_disp)
        {
            break;
        }
    }
    */
    ...
    print_r($t);
    echo "<br>here------------------<br>";
    //21-01-22 - Added below foreach
   /*  [0] => Profession - Automotive Designer
            [1] => cluster Science, Technology, Engineering & Mathematics
            [2] => 11th_12th PCM
            [3] => Education Science with Physics & Maths+Diploma in Computer aided Design(CAD)+B.Des in Transportation/Industrial +M.Des in Transportation/Industrial /Science+Diploma in Automotive Designing/3D +B.Tech in Automotive
            [4] => Recommendation Optional 
           
            SELECT a.Cluster, a.recommendation, a.profession_name,b.11th_12th,b.Education
            Profession Name Cluster 11Th / 12Th Higher Education Recommendation
            */
