...
<div class="content-wrapper bg-white" style="min-height: 706px">
  ...
  <section class="content ">
    <div class="container-fluid">      
          <p class="text-right"><a class="btn btn-sm btn-outline-primary" href="<?php echo base_url('SpController/vocational_training_add');?>">Add New Vocational Training</a></p>
                ...
                <table class="table table-bordered text-nowrap">
                 ...
                 <tr>  
                    ...
                    <td class="text-center">
                      <a class="btn btn-sm btn-outline-primary m-1" href="<?php echo base_url('SpController/vocational_training_details/'.$v->id);?>">Details</a>
                      <?php
                        if($v->training_status == "in_creation"){
                            echo "<a class='btn btn-sm btn-outline-info m-1' href='".base_url('SpController/vocational_training_edit/'.$v->id)."'>Edit</a>";
                    ...
                    <td>
                      <a href="<?php echo base_url('SpController/participant_details/'.$v->id)?>" class='btn btn-sm btn-outline-info'>Click Here</a> 
                    </td>
                </tr> 
...
<script>
  ...
  $(document).on("click",".btn-delete-this",function(){
    ...
    if(confirm("Are you sure you want to delete?")){
        $.get(`${BASE_URL}SpController/delete_itme/${vid}/REMOVE_VOCATIONAL`,function(res){
  ...
  $("#form_cerfifcation_requrest").submit(function(e){
      ...
      fetch(`${BASE_URL}SpController/ajax_work/${e.target.vid.value}/CERTIFICATION_REQUEST`,{
        method:"post",
        body:form_data
      })
  ...
  $(document).on("click",".btn-approval-request",function(){
    ...
    var vid = $(this).data("vid");
    var req_type = $(this).data("req-type");
        $.get(`${BASE_URL}SpController/ajax_work/${vid}/TRAINING_REQUEST?req=${req_type}`,function(res){
          let res_json = JSON.parse(res);          
          if(res_json.MSG == "OK"){
